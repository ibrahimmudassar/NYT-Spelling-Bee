...
soup = BeautifulSoup(
    requests.get("https://www.nytimes.com/puzzles/spelling-bee").content, "html.parser"
)
...
df = pd.DataFrame(
    frequencies.items(), columns=["word", "freq (occurrence per billion)"]
)
df["score"] = df["word"].map(graded)
df["isPangram"] = [ispangram(i, "".join(gameData["validLetters"])) for i in df["word"]]

fig = px.strip(
    df,
    x="freq (occurrence per billion)",
    hover_data=df.columns,
    color="isPangram",
    title=f"NYT Spelling Bee Logarithmic Word Frequency Spread {gameData['printDate']}",
)
...
rare_5 = (
    df.nsmallest(5, "freq (occurrence per billion)")
    .query("isPangram == False")["word"]
    .tolist()
)
...
try:
    ...
    embed.set_thumbnail(url="attachment://result.png")
